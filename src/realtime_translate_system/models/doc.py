...
class Doc(db.Model):  # 繼承 db.Model，定義 ORM 模型
    ...
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.now)  # 建立時間
    update_time = db.Column(db.DateTime, nullable=False, default=datetime.now, onupdate=datetime.now)  # 更新時間
    title = db.Column(db.Text, nullable=False)  # 標題
    keywords = db.Column(db.ARRAY(db.Text), nullable=True)  # 關鍵字（PostgreSQL 陣列類型）
